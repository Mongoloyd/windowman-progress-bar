...
interface FlowCEntryProps {
  quoteStage: 'verbal' | 'appointment';
  onSwitchToA: () => void;
}
...
const FlowCEntry = ({ quoteStage, onSwitchToA }: FlowCEntryProps) => {
  ...
  return (
    <motion.section
      ...
      className="bg-card pt-14 pb-16 px-4 md:pt-20 md:pb-24 md:px-8"
    >
      <div className="mx-auto max-w-4xl">
        ...
        <div className="inline-flex items-center gap-2 rounded-md border border-gold bg-gold-light px-3.5 py-1.5 mb-5">
          ...
          <span className="font-mono text-[11px] font-bold text-amber-text tracking-[0.08em]">
            {isAppointment ? 'CONTRACTOR VISIT APPROACHING — ACT NOW' : "YOU HAVE A NUMBER — LET'S USE IT"}
          </span>
        </div>

        {/* Headline */}
        <h2 className="font-display text-[42px] md:text-[54px] font-extrabold text-navy leading-[1.15]">
          {isAppointment ? 'You have a visit coming up.' : 'You have a number.'}
        </h2>
        <h2 className="font-display text-[42px] md:text-[54px] font-extrabold text-amber-text leading-[1.15]">
          It's not in writing yet. That means you still have leverage.
        </h2>

        {/* Sub */}
        <p className="font-body text-[17px] text-foreground leading-[1.75] max-w-[580px] mt-3.5">
        ...
        <div className="flex flex-col md:flex-row gap-5 max-w-[680px] mb-9">
          {outcomes.map((o, i) => (
            <div key={i} className="flex items-start gap-3">
              <div className={`${o.bg} flex h-9 w-9 shrink-0 items-center justify-center rounded-full`}>
                <span className={`${o.iconColor} text-sm`}>{o.icon}</span>
              </div>
              <div>
        ...
        <p className="font-body text-[13px] text-muted-foreground mt-3.5">
          ...
          <span onClick={onSwitchToA} className="text-cyan-text underline cursor-pointer">
            scan it instead →
          </span>
        </p>
      </div>
