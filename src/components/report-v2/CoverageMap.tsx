import { CheckCircle2, HelpCircle, Minus, Shield, Wrench, DollarSign, FileText, ShieldCheck, UserCheck } from "lucide-react";
...
const STATUS_CONFIG: Record<CoverageStatus, { icon: typeof CheckCircle2; label: string; color: string; bg: string }> = {
  confirmed: { icon: CheckCircle2, label: "Confirmed", color: "text-emerald-400", bg: "bg-emerald-500/10" },
  needs_clarification: { icon: HelpCircle, label: "Needs Clarification", color: "text-amber-400", bg: "bg-amber-500/10" },
  not_visible_in_quote: { icon: Minus, label: "Not in Quote", color: "text-slate-500", bg: "bg-slate-800/50" },
};
  ...
  return (
    <section>
      <div className="mb-4 flex items-center gap-2.5">
        ...
        <h2 className="text-lg font-bold tracking-tight text-white" style={{ letterSpacing: "-0.02em" }}>
          Coverage Analysis
        </h2>
      </div>
        ...
        {rows.map((row) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-3 rounded-xl px-4 py-3.5 bg-slate-900/60 ring-1 ring-white/5 text-left transition-all duration-200 ${isClickable ? "cursor-pointer hover:ring-white/10 hover:bg-slate-900/80" : "cursor-default"}`}
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-white truncate">{domain.label}</p>
