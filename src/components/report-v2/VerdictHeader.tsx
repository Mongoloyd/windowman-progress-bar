...
const GRADE_STYLES: Record<Grade, { ring: string; text: string; bg: string; glow: string }> = {
  A: { ring: "ring-emerald-500", text: "text-emerald-400", bg: "bg-emerald-500/10", glow: "shadow-[0_0_40px_rgba(16,185,129,0.15)]" },
  B: { ring: "ring-cyan-500", text: "text-cyan-400", bg: "bg-cyan-500/10", glow: "shadow-[0_0_40px_rgba(6,182,212,0.15)]" },
  C: { ring: "ring-amber-500", text: "text-amber-400", bg: "bg-amber-500/10", glow: "shadow-[0_0_40px_rgba(245,158,11,0.15)]" },
  D: { ring: "ring-orange-500", text: "text-orange-400", bg: "bg-orange-500/10", glow: "shadow-[0_0_40px_rgba(249,115,22,0.15)]" },
  F: { ring: "ring-red-500", text: "text-red-400", bg: "bg-red-500/10", glow: "shadow-[0_0_40px_rgba(239,68,68,0.15)]" },
};
    ...
    <div className="relative overflow-hidden rounded-2xl bg-slate-900/80 ring-1 ring-white/5">
      ...
      <div className="relative px-6 py-8 sm:px-8 sm:py-10">
        <div className="flex flex-col items-start gap-6 sm:flex-row sm:items-center">
          <div className={`flex h-24 w-24 shrink-0 items-center justify-center rounded-full ring-2 ${gradeStyle.ring} ${gradeStyle.bg} ${gradeStyle.glow} sm:h-28 sm:w-28`}>
            <span className={`text-5xl font-bold tracking-tight ${gradeStyle.text} sm:text-6xl`} style={{ fontFamily: "Inter, system-ui, sans-serif", letterSpacing: "-0.04em" }}>
              {verdict.grade}
            </span>
          </div>
          <div className="flex-1 space-y-3">
            <p className="text-xl font-bold leading-tight tracking-tight text-white sm:text-2xl" style={{ letterSpacing: "-0.02em", lineHeight: "1.15" }}>
              {verdict.stanceLine}
            </p>
            <div className="flex flex-wrap gap-2">
              {verdict.postureChips.map((chip) => {
                ...
                return (
                  <span key={chip} className={`inline-flex items-center rounded-md px-2.5 py-1 text-xs font-semibold uppercase tracking-wider ${style.bg} ${style.text}`}>
          ...
          {mode === "full" && (
            <div className="flex shrink-0 gap-2 sm:flex-col">
              <button onClick={() => console.log("TODO: PDF export")} className="inline-flex items-center gap-2 rounded-lg bg-white/5 px-4 py-2.5 text-sm font-medium text-slate-300 ring-1 ring-white/10 transition-all hover:bg-white/10 hover:text-white">
                <Download className="h-4 w-4" />
                <span className="hidden sm:inline">Download PDF</span>
              </button>
              <button onClick={() => console.log("TODO: Share")} className="inline-flex items-center gap-2 rounded-lg bg-white/5 px-4 py-2.5 text-sm font-medium text-slate-300 ring-1 ring-white/10 transition-all hover:bg-white/10 hover:text-white">
