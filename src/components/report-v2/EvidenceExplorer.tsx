  ...
  return (
    <div className="flex items-start gap-3 rounded-lg bg-slate-800/40 px-4 py-3 ring-1 ring-white/5">
      ...
      <div className="flex-1 min-w-0">
        ...
        {item.sourceText && (
          <pre className="mt-1.5 rounded-md bg-slate-950/50 px-3 py-2 text-xs text-slate-400 font-mono overflow-x-auto whitespace-pre-wrap">{item.sourceText}</pre>
        )}
        ...
        {item.benchmark && (
          <div className="mt-1 flex items-center gap-2">
            ...
            {item.benchmark.region && <span className="text-[10px] text-slate-600">({item.benchmark.region})</span>}
          </div>
        )}
      </div>
      <span className={`shrink-0 rounded-md px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider bg-slate-800 ${kindConfig.color}`}>
        {kindConfig.label}
      </span>
    </div>
  );
...
export function EvidenceExplorer({ evidence, mode, activeFindingFilter }: EvidenceExplorerProps) {
    ...
    return (
      <section className="relative">
        <div className="mb-4 flex items-center gap-2.5">
          ...
          <h2 className="text-lg font-bold tracking-tight text-white" style={{ letterSpacing: "-0.02em" }}>Evidence Explorer</h2>
        </div>
        <div className="relative overflow-hidden rounded-xl bg-slate-900/40 ring-1 ring-white/5">
          <div className="px-6 py-8 text-center">
            ...
            <p className="mt-3 text-sm font-medium text-slate-400">Full source data and extraction evidence available in the unlocked report.</p>
          </div>
        </div>
      </section>
    );
  ...
  const items = filter ? evidence.items.filter((i) => i.findingId === filter) : evidence.items;
  ...
  return (
    <section>
      <div className="mb-4 flex items-center gap-2.5">
        ...
        <h2 className="text-lg font-bold tracking-tight text-white" style={{ letterSpacing: "-0.02em" }}>Evidence Explorer</h2>
        <span className="rounded-full bg-slate-800 px-2 py-0.5 text-xs font-medium text-slate-400">{evidence.items.length} items</span>
      </div>
      {findingIds.length > 1 && (
        <div className="mb-3 flex flex-wrap gap-1.5">
          <button onClick={() => setFilter(null)} className={`rounded-md px-2.5 py-1 text-xs font-medium transition-colors ${!filter ? "bg-cyan-500/15 text-cyan-400 ring-1 ring-cyan-500/30" : "bg-slate-800 text-slate-400 hover:text-slate-300"}`}>All</button>
          {findingIds.map((id) => (
            <button key={id} onClick={() => setFilter(id === filter ? null : id)} className={`rounded-md px-2.5 py-1 text-xs font-medium transition-colors ${filter === id ? "bg-cyan-500/15 text-cyan-400 ring-1 ring-cyan-500/30" : "bg-slate-800 text-slate-400 hover:text-slate-300"}`}>
